...
<div style="max-width: 40rem; margin: 0 auto;">
  ...
  <div style="border-radius: 0.75rem; border: 1px solid hsl(var(--border)); background: hsl(var(--card)); padding: 1.5rem;">
    ...
    <form method="post" action="{routePath('contact-us')}" style="display: flex; flex-direction: column; gap: 1.25rem;">
      <input type="hidden" name="token" value="{$token}" />

      <div>
        <label style="display: block; margin-bottom: 0.375rem; font-size: 0.875rem; font-weight: 500;">Your Name</label>
        <input type="text" name="name" value="{$name}" class="venom-input" placeholder="John Doe" required />
      </div>

      <div>
        <label style="display: block; margin-bottom: 0.375rem; font-size: 0.875rem; font-weight: 500;">Email Address</label>
        <input type="email" name="email" value="{$email}" class="venom-input" placeholder="you@example.com" required />
      </div>

      <div>
        <label style="display: block; margin-bottom: 0.375rem; font-size: 0.875rem; font-weight: 500;">Subject</label>
        <input type="text" name="subject" value="{$subject}" class="venom-input" placeholder="How can we help?" required />
      </div>

      <div>
        <label style="display: block; margin-bottom: 0.375rem; font-size: 0.875rem; font-weight: 500;">Message</label>
        <textarea name="message" class="venom-input" rows="6" placeholder="Tell us more…" required style="resize: vertical;">{$message}</textarea>
      </div>
