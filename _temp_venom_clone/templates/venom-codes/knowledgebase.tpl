...
<div style="max-width: 48rem; margin: 0 auto;">
  ...
  {* Search *}
  <form method="get" action="{$WEB_ROOT}/knowledgebase.php" style="margin-bottom: 2rem;">
    <div style="position: relative; max-width: 32rem; margin: 0 auto;">
      ...
      <input type="text" name="search" value="{$search}" class="venom-input" placeholder="Search articles…" style="padding-left: 2.25rem; padding-right: 1rem;" />
    </div>
  </form>
    ...
    <div style="display: grid; gap: 1rem; grid-template-columns: repeat(1, 1fr);">
      ...
      <div class="kb-grid" style="display: grid; gap: 1rem; grid-template-columns: repeat(1, 1fr);">
        {foreach $kbcategories as $category}
          <a href="{$WEB_ROOT}/knowledgebase/{$category.id}/{$category.urlfriendlyname}" style="text-decoration: none; color: inherit; display: block; border-radius: 0.75rem; border: 1px solid hsl(var(--border)); background: hsl(var(--card)); padding: 1.25rem; transition: border-color 0.15s;">
            ...
            {/if}
            <span class="text-xs text-muted-foreground">{$category.numarticles} {if $category.numarticles == 1}article{else}articles{/if}</span>
          </a>
        {/foreach}
      </div>
    </div>
  {else}
    <div style="border-radius: 0.75rem; border: 1px dashed hsl(var(--border)); background: hsl(var(--card)); padding: 3rem; text-align: center;">
      <p class="text-muted-foreground" style="font-size: 0.875rem;">No knowledge base categories found.</p>
    </div>
  {/if}
